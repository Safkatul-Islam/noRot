import type { Persona, Severity, UsageCategories } from '@norot/shared';

function getTarget(categories: UsageCategories): string | null {
...
type PersonaMessages = Record<1 | 2 | 3 | 4, (target: string, what: string) => string>;

type PersonaVariants = Record<1 | 2 | 3 | 4, Array<(target: string, what: string) => string>>;

const personaVariantMap: Record<Persona, PersonaVariants> = {
  calm_friend: {
    1: [
...
function pickVariant(variants: Array<(target: string, what: string) => string>, key: string): (target: string, what: string) => string {
...
export function buildInterventionText(
  severity: Severity,
  persona: Persona,
  ...
  const level = (severity >= 1 && severity <= 4 ? severity : 1) as 1 | 2 | 3 | 4;
