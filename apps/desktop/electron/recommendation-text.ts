...
export function buildStableRecommendationText(severity: Severity, persona: Persona): string {
  ...
  const messages: Record<Persona, Record<string, string>> = {
    calm_friend: {
      ...
      interrupt:
        "I care about you — and right now you're deep in procrastination. Take one breath, then come back to your task.",
      crisis:
        "I'm worried you're spiraling. Step away for a few minutes, drink water, then come back with one tiny next step.",
    },
    coach: {
