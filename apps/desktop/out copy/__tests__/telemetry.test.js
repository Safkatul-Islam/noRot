...
describe('classifyApp', () => {
    ...
    it('uses first-match-wins ordering', () => {
        const customRules = [
            { id: '1', matchType: 'app', pattern: 'Code', category: 'entertainment' },
            { id: '2', matchType: 'app', pattern: 'Code', category: 'productive' },
        ];
        expect(classifyApp('Code', customRules)).toBe('entertainment');
    });
    it('skips title-type rules', () => {
        const titleRules = [
            { id: '1', matchType: 'title', pattern: 'Code', category: 'productive' },
        ];
