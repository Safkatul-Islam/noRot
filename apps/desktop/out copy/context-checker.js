...
// --- Main export ---
export async function checkContextRelevance(apiKey, appName, windowTitle, domain, activeTodos) {
    // 1. Filter to active todos that have allowedApps
    const todosWithApps = activeTodos.filter((t) => !t.done && t.allowedApps && t.allowedApps.length > 0);
    if (todosWithApps.length === 0)
        return null;
    // 2. Check if any todo's allowedApps match current app or domain
    const lowerApp = appName.toLowerCase();
    const lowerDomain = (domain ?? '').toLowerCase();
    const matched = todosWithApps.find((t) => t.allowedApps.some((allowed) => {
