...
export function registerIpcHandlers() {
    // --- App ---
    ipcMain.handle(IPC_CHANNELS.RELAUNCH_APP, (_event, rendererUrl) => {
        ...
        if (typeof rendererUrl === 'string' && rendererUrl.startsWith('http')) {
            ...
            if (!hasRendererUrlArg)
                nextArgs.push(`--renderer-url=${rendererUrl}`);
        }
    ...
    // --- Interventions ---
    ipcMain.handle(IPC_CHANNELS.RESPOND_TO_INTERVENTION, (_event, eventId, response) => {
        orchestrator.handleInterventionResponse(eventId, response);
    });
    // --- Audio Played ---
    ipcMain.handle(IPC_CHANNELS.REPORT_AUDIO_PLAYED, (_event, interventionId) => {
    ...
    // --- App Stats ---
    ipcMain.handle(IPC_CHANNELS.GET_APP_STATS, (_event, minutes) => {
    ...
    ipcMain.handle(IPC_CHANNELS.UPDATE_SETTINGS, (_event, settings) => {
        ...
        const nextToughLoveAllowed = typeof settings.toughLoveExplicitAllowed === 'boolean'
            ? settings.toughLoveExplicitAllowed
            : currentSettings.toughLoveExplicitAllowed;
        const nextPersona = typeof settings.persona === 'string'
            ? settings.persona
    ...
    ipcMain.handle(IPC_CHANNELS.TEST_INTERVENTION, async () => {
        ...
        let text = buildInterventionText(1, settings.persona, testCategories);
        // Try Gemini for a dynamic script, fall back to hardcoded text
        if (settings.scriptSource === 'gemini' && settings.geminiApiKey) {
            const geminiText = await generateScript(settings.geminiApiKey, 1, settings.persona, settings.toughLoveExplicitAllowed);
            if (geminiText)
                text = geminiText;
        }
        const intervention = {
        ...
        if (text) {
            const tts = { model: 'eleven_v3', stability: 50, speed: 1.0 };
    ...
    ipcMain.handle(IPC_CHANNELS.ELEVENLABS_TTS, async (_event, payload) => {
        const settings = database.getSettings();
    ...
    ipcMain.handle(IPC_CHANNELS.EXTRACT_TODOS, async (_event, transcript) => {
        const settings = database.getSettings();
        if (!settings.geminiApiKey)
            return [];
    ...
    ipcMain.handle(IPC_CHANNELS.ADD_TODO, (_event, item) => {
        database.addTodo(item);
        clearContextCache();
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.TOGGLE_TODO, (_event, id) => {
        database.toggleTodo(id);
        clearContextCache();
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.DELETE_TODO, (_event, id) => {
        database.deleteTodo(id);
        clearContextCache();
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.UPDATE_TODO, (_event, id, fields) => {
        database.updateTodo(id, fields);
        clearContextCache();
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.REORDER_TODOS, (_event, id, newOrder) => {
        database.reorderTodo(id, newOrder);
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.SET_TODOS, (_event, items) => {
        database.setTodos(items);
        clearContextCache();
        broadcastTodos();
    });
    ipcMain.handle(IPC_CHANNELS.APPEND_TODOS, (_event, items) => {
    ...
    ipcMain.on(IPC_CHANNELS.BROADCAST_VOICE_STATUS, (event, payload) => {
