...
import { calculateScore, applySnoozeEscalation, generateReasons, SEVERITY_BANDS, SNOOZE_PRESSURE_POINTS, SNOOZE_PRESSURE_DURATION_MIN, MAX_SNOOZE_PRESSURE, } from '@norot/shared';
...
function resolveTimeZoneSetting(timeZoneSetting) {
    ...
    if (!timeZoneSetting || timeZoneSetting === 'system')
        return system || 'UTC';
...
const state = {
    ...
    snoozePressure: 0,
    snoozeTimestamps: [],
    currentSettings: null, // Lazily loaded after DB init
...
async function processSnapshot(snapshot) {
    try {
        ...
        const apiScoreResult = await apiClient.scoreSnapshot(snapshot, snoozePressure, settings.persona);
        ...
        scoreResult.recommendation.text = buildStableRecommendationText(scoreResult.severity, scoreResult.recommendation.persona);
        // Store the score
        ...
        if (meetsThreshold && cooldownExpired && hasRecommendation) {
            ...
            if (settings.scriptSource === 'gemini' && settings.geminiApiKey) {
                ...
                if (snapshot.categories.contextOverride || snapshot.categories.contextTodo || overdueTodos.length > 0) {
                    interventionText = await generateContextAwareScript(settings.geminiApiKey, scoreResult.severity, scoreResult.recommendation.persona, {
                    ...
                    interventionText = await generateScript(settings.geminiApiKey, scoreResult.severity, scoreResult.recommendation.persona, settings.toughLoveExplicitAllowed);
                }
            }
            // Fall back to on-device script (also used when scriptSource is 'default')
            if (!interventionText) {
                interventionText = buildInterventionText(scoreResult.severity, scoreResult.recommendation.persona, snapshot.categories, overdueTodos.length > 0 ? overdueTodos : undefined);
            }
            ...
            console.log(`[orchestrator] INTERVENTION FIRED: id=${intervention.id} score=${intervention.score} severity=${intervention.severity}`);
...
export function startTelemetry() {
    ...
    state.currentSettings = database.getSettings();
    settingsLoaded = true;
    console.log(`[orchestrator] Settings loaded: threshold=${state.currentSettings.scoreThreshold} cooldown=${state.currentSettings.cooldownSeconds}s persona="${state.currentSettings.persona}" rulesCount=${state.currentSettings.categoryRules.length} geminiKey=${!!state.currentSettings.geminiApiKey}`);
    ...
    state.activeInterventionCategories = null;
    state.complianceSnapshots = 0;
    state.telemetryCollector = createTelemetryCollector(() => ensureSettings().categoryRules, () => ensureSettings().visionEnabled, processSnapshot, () => ensureSettings().geminiApiKey, () => database.getTodos().filter((t) => !t.done));
...
export function handleInterventionResponse(eventId, response) {
...
export function refreshSettings() {
    ...
    settingsLoaded = true;
    clearContextCache();
    console.log(`[orchestrator] Settings refreshed: threshold=${state.currentSettings.scoreThreshold} cooldown=${state.currentSettings.cooldownSeconds}s`);
}
