...
export function useInterventions() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = api.onIntervention((event) => {
        ...
        const unsubDismiss = api.onInterventionDismiss?.((data) => {
            const { interventionId } = data;
            setActiveIntervention((cur) => (cur?.id === interventionId ? null : cur));
            setInterventions((prev) => prev.map((item) => item.id === interventionId ? { ...item, userResponse: 'dismissed' } : item));
        });
        return () => {
            if (typeof unsubscribe === 'function')
                unsubscribe();
            if (typeof unsubDismiss === 'function')
                unsubDismiss();
        };
    }, []);
    const respondToIntervention = useCallback(async (id, response) => {
        const api = getNorotAPI();
        await api.respondToIntervention(id, response);
        setInterventions((prev) => prev.map((item) => item.id === id ? { ...item, userResponse: response } : item));
