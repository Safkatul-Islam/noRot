...
export const mockNorotAPI = {
    ...
    sendChatMessage: (_message, _sessionId) => {
        const response = MOCK_CHAT_RESPONSES[chatMsgIndex % MOCK_CHAT_RESPONSES.length];
        chatMsgIndex++;
        let i = 0;
        const interval = setInterval(() => {
            if (i < response.length) {
                for (const cb of chatTokenListeners)
                    cb(response[i]);
                i++;
            }
            else {
                clearInterval(interval);
                for (const cb of chatDoneListeners)
                    cb();
            }
        }, 20);
    },
    ...
    extractTodos: async (_transcript) => [
        {
            id: 'mock-todo-1',
    ...
    updateTodo: async (id, fields) => {
        ...
        if ('allowedApps' in fields) {
            if (Array.isArray(fields.allowedApps))
                todo.allowedApps = fields.allowedApps;
            else
                delete todo.allowedApps;
        }
    ...
    reorderTodos: async (id, newOrder) => {
    ...
    setTodos: async (items) => {
        mockTodos = [...items];
        broadcastTodos();
    },
    appendTodos: async (items) => {
    ...
    broadcastVoiceStatus: (_isSpeaking, _severity, _amplitude, _lastWordBoundaryAt) => { },
    onVoiceStatus: () => () => { },
...
let chatTokenListeners = [];
let chatDoneListeners = [];
let chatErrorListeners = [];
