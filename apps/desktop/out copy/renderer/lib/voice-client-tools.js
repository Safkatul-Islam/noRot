 ...
 */
export function findTodoByText(todos, searchText) {
...
export function createTodoClientTools(logPrefix) {
    return {
        list_todos: async (params = {}) => {
            try {
        ...
        update_todo: async (params) => {
            try {
                ...
                if (!todo)
                    return 'Could not find a matching task. Ask the user to clarify which task.';
                const fields = {};
                if (params.new_text)
                    fields.text = params.new_text;
                if (params.deadline)
                    fields.deadline = params.deadline;
                if (params.app)
                    fields.app = params.app;
