...
function hasDigitalCue(todo) {
    if (typeof todo.url === 'string' && todo.url.trim())
        return true;
...
export function isTodoLikelyOffline(todo) {
    const t = normalizeText(todo.text);
    if (!t)
        return true;
    if (HARD_OFFLINE_PATTERNS.some((re) => re.test(t)))
        return true;
    const softMatch = SOFT_OFFLINE_PATTERNS.some((re) => re.test(t));
    if (softMatch)
        return !hasDigitalCue(todo);
    return false;
}
export function filterComputerScopedTodos(todos) {
