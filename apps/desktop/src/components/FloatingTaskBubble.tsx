...
interface FloatingTaskBubbleProps {
...
export function FloatingTaskBubble({ bubble, index, onSettle }: FloatingTaskBubbleProps) {
  const time = useTime();
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        <motion.div
          layoutId={`task-bubble-${bubble.id}`}
          style={{ x, y }}
          ...
          className={cn(
            'max-w-[220px] px-3 py-1.5 rounded-full',
            'text-xs font-medium text-text-primary/95 truncate',
            'bg-[var(--color-glass)] backdrop-blur-xl',
            'border border-primary/30',
            'shadow-[0_14px_40px_-26px_rgba(0,0,0,0.95),0_0_26px_-18px_var(--color-glow-primary)]',
          )}
          title={bubble.text}
        >
          {bubble.text}
