export type VoiceErrorCode = 'NO_API_KEY' | 'AUTH' | 'RATE_LIMIT' | 'NETWORK' | 'UNKNOWN';
...
const VALID_CODES = new Set<string>(['NO_API_KEY', 'AUTH', 'RATE_LIMIT', 'NETWORK', 'UNKNOWN']);

const FALLBACK_ERROR: VoiceAgentError = {
  code: 'UNKNOWN',
 ...
 */
export function parseVoiceError(err: unknown): VoiceAgentError {
  const raw = err instanceof Error ? err.message : String(err);
  try {
    const jsonStart = raw.indexOf('{');
    const jsonEnd = raw.lastIndexOf('}');
    if (jsonStart === -1 || jsonEnd <= jsonStart) return { ...FALLBACK_ERROR };
    ...
    if (
      ...
      return {
        code: parsed.code as VoiceErrorCode,
