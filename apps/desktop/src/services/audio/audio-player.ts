...
export class AudioPlayer {
  ...
  private currentMediaNode: MediaElementAudioSourceNode | null = null;
  ...
  async prime(): Promise<void> {
    ...
    try {
      ...
      if (this.gainNode) this.gainNode.gain.value = prevGain;
    }
  }
  ...
  private resolvePlaybackUrl(url: string): string {
    const raw = (url ?? '').trim();
    if (!raw) return raw;
