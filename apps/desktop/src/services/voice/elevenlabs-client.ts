import type { TTSSettings } from '@norot/shared';
  ...
  async synthesize(
    ...
    if (typeof api.synthesizeElevenLabsTts === 'function') {
      try {
        ...
        try {
          const parsed = JSON.parse(msg) as { statusCode?: number; message?: string };
    ...
    console.log('[ElevenLabs] synthesize: model=%s stability=%s speed=%s voiceId=%s textLen=%d',
      modelId, stability, settings.speed, voiceId, text.length);
    ...
    if (!contentType.includes('audio/')) {
      throw new ElevenLabsClientError(
        `ElevenLabs returned non-audio content-type: ${contentType}`
      );
    }
